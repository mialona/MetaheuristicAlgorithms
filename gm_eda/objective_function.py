...
def max_objective_function(data, sigma, M3_collision):
    """
    The 'max_objective_function' function calculates the maximum efficiency
    percentage value among all the possible cuts for the given permutation.

    Args:
        data: List with the values of the operations.
        sigma: Solution/permutation.
        M3_collision: Minimum distance between heads.
    ...
    # All possible cuts of the solution are checked
    for i in range(1, len(sigma)):
        # Call to objective function
        percent, d_head_a, d_head_b = objective_function(data, sigma[0:i], sigma[i:len(sigma)], M3_collision)

        # The best value obtained and its corresponding distribution of operations are stored
        if(percent > max_percent):
            max_percent = percent
            max_ops_a = sigma[0:i]
            max_ops_b = sigma[i:len(sigma)]

    # Return results
    return max_percent, max_ops_a, max_ops_b